...
const AccountProfile = ({ user, btnTitle }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof userValidaton>) => {
          ...
          render={({ field }) => (
            <FormItem className="flex items-center gap-4">
              <FormLabel className="account-form_image-label">
                {field.value ? <Image src={field.value} alt="profile photo" width={96} height={96} priority className="rounded-full object-contain" /> : <AiOutlineFileImage className="w-6 h-6 object-contain text-white" />}
              </FormLabel>
